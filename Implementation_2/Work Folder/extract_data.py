...
extract_file_path = base_path / "diabetes" / "diabetes-data.tar.Z"


# check if directory for extract data exists, if not create it
if Path(base_path / "extracted_data").is_dir():
    print("Extracted data directory exists")
else:
    Path(base_path / "extracted_data").mkdir(parents=True, exist_ok=True)
...
if Path(base_path / "transformed_data").is_dir():
    ...
    Path(base_path / "transformed_data").mkdir(parents=True, exist_ok=True)
    ...
    with tarfile.open(fileobj=io.BytesIO(decompressed_data), mode="r") as tar:
        for i, member in enumerate(tar.getmembers()):
            if member.isfile():
                ...
                df.to_csv(base_path / "extracted_data" / clean_name, index=False)
...
for i, file in enumerate(Path(base_path / "extracted_data").glob("*.csv")):
    df = pd.read_csv(file)
    print(i)
    if i in range(0,70):
        df['PATIENT_ID'] = i+1
        ...
        #print(df.tail(5))
        #print(df.dtypes)
        # Perform transformation (example: unstacking)
        #transformed_df = df.pivot_table(index=['PATIENT_ID', 'Date', 'Time'], columns='Code', values='Value').reset_index()
        #transformed_df.set_index("PATIENT_ID", inplace=True)
        
        # Save transformed data
        clean_name = f"{file.name.replace('.csv', '')}_transformed.csv"
        df.to_csv(base_path / "transformed_data" / clean_name, index=True)